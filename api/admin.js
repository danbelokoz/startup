...
const PARSERS = [
  ...
  { id:'onsale', name:'Листинги на продаже — обновление', source:'GitHub Actions', scheduleText:'Каждые 3 ч (01–22 UTC)', sched:{ m:47, h:[1,4,7,10,13,16,19,22] }, maxRunMin:110,
    desc:'Обновляет стартапы на продаже поштучно, ПО ИМЕНИ — так потолок в 200 записей не действует. За 8 прогонов в сутки обходит все ~1990 листингов. Ловит проданные и удалённые: если TrustMRR больше не знает стартап, он перестаёт числиться в продаже. Источник чередуется по дням: один день API, другой — машиночитаемая страница /startup/имя.md (она не тратит квоту и работает в 10 раз быстрее). Какой источник был — написано в деталях запуска.' },
