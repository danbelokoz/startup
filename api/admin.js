...
const PARSERS = [
  ...
  { id:'onsale', name:'Листинги на продаже — обновление', source:'GitHub Actions', scheduleText:'Каждые 3 ч (01–22 UTC)', sched:{ m:47, h:[1,4,7,10,13,16,19,22] }, maxRunMin:110,
    desc:'Обновляет стартапы на продаже поштучно, запрашивая каждый ПО ИМЕНИ — так потолок в 200 записей не действует. За 8 прогонов в сутки обходит все ~1990 листингов, поэтому цены и статусы не залёживаются. Заодно ловит проданные и удалённые: если TrustMRR больше не знает такой стартап, он перестаёт числиться в продаже. Использует половину квоты (5 запросов/мин), вторая остаётся живому сайту.' },
